...
class TBCL():
    ...
    def read_in_characters(self):
        ...
        with open('data/TBCL-20220920-characters-scrape.html', encoding='utf-8') as input_file:
            soup = BeautifulSoup(input_file, 'html.parser')
            entries = soup.find_all("tr", class_='dash')

            for row in entries:
                parts = row.find_all("td")
                word = parts[1].contents[0]
                level = parts[3].contents[0]
